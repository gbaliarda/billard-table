...
def generate_static_file():
    with open(STATIC_FILE, "w") as f:
        # White Ball
        f.write(f"{WHITEBALL_COORDS[0]} {WHITEBALL_COORDS[1]} {WHITEBALL_VX} 0 {BALL_MASS} {BALL_DIAMETER/2}\n")

        # Fixed balls (table holes)
        f.write(f"0 0 0 0 0 {BALL_DIAMETER}\n")
        f.write(f"{TABLE_WIDTH / 2} 0 0 0 0 {BALL_DIAMETER}\n")
        f.write(f"{TABLE_WIDTH} 0 0 0 0 {BALL_DIAMETER}\n")
        f.write(f"0 {TABLE_HEIGHT} 0 0 0 {BALL_DIAMETER}\n")
        f.write(f"{TABLE_WIDTH / 2} {TABLE_HEIGHT} 0 0 0 {BALL_DIAMETER}\n")
